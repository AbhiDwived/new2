...
export default function WeddingMakeUp() {
  ...
  // console.log("choreographers",data);

  // const choreographers = data?.vendors?.filter(v => v.vendorType === "Choreographers");
  const choreographers = useMemo(() => {
    return data?.vendors?.filter(v => v.vendorType === "Choreographers") || [];
  }, [data]);
  ...
  useEffect(() => {
    if (!choreographers) return;

    let sorted = [...choreographers];
