...
export default function MakeupArtists() {
  ...
  const [selectedCategory, setSelectedCategory] = useState("Makeup Artists");
  ...
  const makeupArtists = useMemo(() => {
    return data?.vendors?.filter(v => v.vendorType === "Makeup Artists") || [];
  }, [data]);

  useEffect(() => {
    if (!makeupArtists) return;

    let sorted = [...makeupArtists];
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      {filteredVendors?.length === 0 ? (
        <p className="text-center col-span-full">No makeup artists available.</p>
      ) : (
