...
export default function MehndiArtists() {
  const router = useRouter();
  const [selectedCategory, setSelectedCategory] = useState("Mehndi Artists");
  ...
  const mehndiArtists = useMemo(() => {
    return data?.vendors?.filter(v => v.vendorType === "Mehndi Artists") || [];
  }, [data]);

  useEffect(() => {
    if (!mehndiArtists) return;

    let sorted = [...mehndiArtists];
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      {filteredVendors?.length === 0 ? (
        <p className="text-center col-span-full">No mehndi artists available.</p>
      ) : (
            ...
            <div className="flex flex-col justify-between flex-grow p-2">
              ...
              <div className="border-t mt-3 pt-3 text-sm text-gray-800">
                ...
                <div className="flex flex-wrap gap-3 text-xs text-gray-600">
                  <span className="text-gray-600 hover:underline p-1 rounded" onClick={() => handleVendorClick(vendor._id)}>
                    {(() => {
