...
export default function VendorPromotion() {
    ...
    // const vendorPromotion = data?.vendors?.filter(v => v.vendorType === "VendorPromotion");
    // console.log("vendorPromotion", vendorPromotion);

    const vendorPromotion = useMemo(() => {
        return data?.vendors?.filter(v => v.vendorType === "VendorPromotion") || [];
    }, [data]);
    ...
    useEffect(() => {
        if (!vendorPromotion) return;

        let sorted = [...vendorPromotion];
